...
from object_removal.io.compare_run import (
    CompareRunContext,
    env_for,
    expand_davis_tasks,
    load_pipelines_yaml,
    resolve_compare_context,
    resolve_davis_paths,
)
...
def _collect_required_envs(ctx: CompareRunContext) -> List[str]:
    ...
    for name in ctx.pipeline_ids:
        ...
        skip_mask_track = bool(isinstance(share_raw, str) and share_raw.strip())
        if not skip_mask_track and (st is None or st == "mask"):
            req.append(env_for(ctx.env_map, stage="mask", method=str(spec["mask"])))
        if not skip_mask_track and (st is None or st == "track"):
...
def _run_pipelines_for_task(
    ctx: CompareRunContext,
    fast_vqa_eval_kw: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """Run all pipeline stages for one task (sequence). Returns one metrics row per pipeline."""
    task = ctx.task
...
def _average_rows(seq_rows_map: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    """Average metrics across sequences, per pipeline method."""
    averaged: List[Dict[str, Any]] = []
    for method, row_list in sorted(seq_rows_map.items()):
        avg: Dict[str, Any] = {"method": method}
        for col in _METRIC_COLUMNS:
            vals = [r.get(col) for r in row_list if isinstance(r.get(col), (int, float))]
            avg[col] = sum(vals) / len(vals) if vals else ""
        averaged.append(avg)
    return averaged
